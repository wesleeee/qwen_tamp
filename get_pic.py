...
from scipy.stats import gaussian_kde   # SciPy 自带

prompts,images=get_test_inputs()
model_path="/home/chuziyuan/zju/TAMP/checkpoints/VLM-R1-Qwen2.5VL-3B-Math-0305"

original_weights=extract_qkv_weights_for_pruning(model_path)

activated_weights,sparsity=process_multiple_input(original_weights,prompts,images,model_path)

original_w=np.abs(np.array(original_weights['Layer_14']['Value Linear Layer']).flatten())
activated_w=np.array(activated_weights['Layer_14']['Value Linear Layer']).flatten()
# 1. 先保留真正的权重副本
activated_val = activated_w.copy()         # 假设这里仍是权重值
activated_idx = np.arange(len(activated_w))

# 2. 随机抽样并保持原顺序
sample_idx = np.random.choice(len(activated_w), size=2000, replace=False)
sample_idx.sort()

# 3. 取子样本的数据
sample_act_val = activated_val[sample_idx]      # 激活后权重值
sample_ori_val = original_w[sample_idx]         # 对应的原始权重值

# 4. 把这 1000 个值各自转成秩 (argsort 两次)
rank_act = np.argsort(np.argsort(sample_act_val))
rank_ori = np.argsort(np.argsort(sample_ori_val))
...
matplotlib.use("Agg")
import matplotlib.pyplot as plt
from scipy.stats import gaussian_kde   # 若无 SciPy 可改为 hexbin

# rank_ori、rank_act 已在作用域中
assert len(rank_ori) == len(rank_act)
max_rank = max(rank_ori.max(), rank_act.max())

# ---------- 1. rank-rank 散点（密度着色） ----------
xy   = np.vstack([rank_ori, rank_act])
dens = gaussian_kde(xy)(xy)
order = dens.argsort()
x, y, dens = rank_ori[order], rank_act[order], dens[order]

# ---------- 2. 计算 IoU(Ĥ) ----------
# 目标：≈ 50 个采样点
step = max_rank // 50 or 1
thresholds = np.arange(step, max_rank + 1, step)
t_ratio    = thresholds / max_rank      # 横坐标 0–1
...
sc  = ax0.scatter(x, y, c=dens, s=16, cmap="viridis", linewidths=0)
fig.colorbar(sc, ax=ax0, label="Point density")
ax0.set_title("Rank–rank scatter (density-coloured)")
ax0.set_xlabel("rank(original weights)")
ax0.set_ylabel("rank(activated weights)")
ax0.grid(True)

# (b) 下半：IoU-曲线（横坐标 0-1）
ax1 = fig.add_subplot(gs[1])
ax1.plot(t_ratio, ious, marker="o", markersize=4, label="IoU")  # markersize=4≈s=8
ax1.set_xlabel("Relative threshold  T / Tmax")
ax1.set_ylabel("IoU (intersection / union)")
ax1.set_title("IoU curve vs. relative threshold")
ax1.set_xlim(0, 1.02)
ax1.set_ylim(0, 1.05)
ax1.grid(True)
ax1.legend()
...
plt.tight_layout()
plt.savefig("/home/chuziyuan/zju/qwen_tamp/compare_combined.png")
print("Saved: compare_combined.png")
